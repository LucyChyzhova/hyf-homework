const movies = [
  { title: "'71", year: 2014, rating: 7, votes: 41702, running_times: 5940 },
  {
    title: "'A' gai wak",
    year: 1983,
    rating: 7.4,
...
const shortestMovieTitle = movies.filter(
  movieTitle => movieTitle.title.length <= 4
);
console.log(shortestMovieTitle);

// 2. Create an array of movie titles with long movie titles
const longestMovieTitle = movies.filter(
  movieTitle => movieTitle.title.length > 50
);
console.log(longestMovieTitle);

// 3. Count the number of movies made between 1980-1989 (including both the years)

const movieNinetiesMinus = movies
  .filter(movie => movie.year >= 1980 && movie.year <= 1989)
  .map(movie => movie.year);

console.log(movieNinetiesMinus);
console.log("We have " + movieNinetiesMinus.length +" movies which were made between 1980-1989");

// 4. Create a new array that has an extra key called tag. The tag is based on the rating: Good (>= 7), Average (>= 4 and < 7), Bad (< 4)

function addTagToMovies(movies) {
  movies.forEach(movie => {
    if (movie.rating >= 7) {
      movie.tag = "Good";
    } else if (movies.rating < 4) {
      movie.tag = "bad";
    } else if (movies.rating >= 4 && movies[i].rating < 7) {
      movie.tag = "Avarage";
    }
  });
  return movies;
...
const moviesWithTag = addTagToMovies(movies);
...
const moviesRatingHigherThan6= moviesWithTag.filter(
    movieRating => movieRating.rating >= 6
  );
  console.log(moviesRatingHigherThan6);

const moviesRatingOnly = moviesRatingHigherThan6.map(movie => movie.rating)
console.log(moviesRatingOnly)
...
function containsSpecificWord(movie,word) {
    const lowCaseWord = word.toLowerCase();
  return movie.title.toLowerCase().includes(lowCaseWord);
}

   const countKeywords = movies.filter(
     movie =>
       containsSpecificWord(movie, "surfer") ||
       containsSpecificWord(movie, "alien") ||
       containsSpecificWord(movie, "benjamin")
   );

   console.log(countKeywords);
   console.log("There are " + countKeywords.length + " movies");
...
function areThereDuplicateWords(movieTitle) {
  let movieTitlesWords = movieTitle.split(" ");
  let duplicates = movieTitlesWords.filter((item, index) => movieTitlesWords.indexOf(item) != index);
  let containsDuplicates = duplicates.length > 0;

  return containsDuplicates;
}
const moviesWithDuplicateWords = movies.filter(movie => areThereDuplicateWords(movie.title));
